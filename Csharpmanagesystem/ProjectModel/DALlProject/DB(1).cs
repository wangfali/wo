...
namespace DALProject
{
    ...
    public class DBSQL 
    {
        ...
        private string strService = "WINDOWS\\WPHSQL";  // 定义服务器名称
        private string strDataBase = "TestDB"; //定义数据库名称
        private string strUser = "sa";  //定义登陆用户名
        private string strPassword = "111111"; //定义登陆用户密码
        private int intType = 1;  //定义连接类型  
        private DBSQLTYPE dbConnectType = DBSQLTYPE.WIN ;//数据库类别，是web还是win
        ...
        public DBSQL()
        {
            if (dbConnectType == DBSQLTYPE.WIN)
            {
                ...
                strConnect = "";// System.Configuration.ConfigurationManager.AppSettings["connectionStrings"];
        }
        /// <summary>
        /// 构造方法
        /// </summary>
        /// <param name="dbType">连接类别，是Web还是Win方式，默认Web</param>
        public DBSQL(DBSQLTYPE dbType)
        {
            ...
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            case 0: //远程连接
                                strConnect = "Data Source=" + serviceName + ",1433;Network Library=DBMSSOCN;Initial Catalog=" + databaseName + ";User Id=" + userName + ";password=" + password + ";Connect Timeout=10;";
                                break;
                            case 1://本地连接
                        ...
                        //strConnect = @"Data Source=WINDOWS\WPHSQL;Initial Catalog=TestDB;Integrated Security=True";
                        sqlConnect.ConnectionString = strConnect;                       
                        return sqlConnect;
                    }
                    catch (Exception)
        ...
        public string sqlCommandByReturn1(SqlCommand cmd)
        {
            try
            {
                SqlConnection sqlConnect = null;
                ...
                if (sqlConnect != null) //数据库连接成功
                {
                    ...
                    {
                        cmd.CommandType = CommandType.StoredProcedure; //设置命令类型为存储过程                    
                        SqlParameter returnValue = new SqlParameter("@return", SqlDbType.NVarChar, 32000);
        ...
        public DataTable dbToDT(string strCmd,bool isProc)
        {
            DataSet ds = null;
            if (isProc)
                ds=dbToDS(strCmd, "temp");
            else
                ds = dbToDS(strCmd, "temp", false);
            if (ds == null)
                return null;
            else
                return ds.Tables["temp"];
        ...
        /// <param name="cmd"></param>
        /// <returns></returns>
        public DataTable dbToDT(SqlCommand cmd)
        {
            try
            {
                sqlConnect = getConnection();
                ...
                if (sqlConnect != null) //连接成功
                {
                    ...
                    if (sqlConnect.State == ConnectionState.Open)
                    {
                        ...
                        DataSet ds = new DataSet(); //创建数据集对象                   
                        ds.Clear();
                        sqlDA.Fill(ds, "temp"); //填充数据集
                        if (ds != null && ds.Tables["temp"] != null)
                            return ds.Tables["temp"];
                        else
                            return null;
                    }
